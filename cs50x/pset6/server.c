...
char* indexes(const char* path)
{
    ...
    // allocate space for index.php file
    full_path = malloc(strlen(path) + 10);
    
    // copy the contents of 'path' into 'full_path'
    strcpy(full_path, path);
    
    // insert the full path in 'full_path'
    ...
    // free the stored value in 'full_path'
    free(full_path);
    
    // allocate space for index.html file
    full_path = malloc(strlen(path) + 11);
    ...
    // free 'full_path' because the files do not exist
    free(full_path);
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    *length = 0;
    ...
    // allocate space for contents of file
    *content = malloc(*length);
    
    // set file pointer to the start of file
    fseek(file, 0, SEEK_SET);
    
    // store the contents of file in 'content'
    ...
    for (int i = 0; path[i] != '\0'; ++i) {
        if (path[i] == '.') {
            if (path[i + 3] == '\0') {
                ...
                // store the file extension into 'ext'
                for (int j = i + 1; path[j] != '\0'; ++j) {
                    ext[j - i - 1] = path[j];
                }
                ...
                if (strcasecmp(ext, "js") == 0) {
                    type = malloc(strlen("text/javascript0"));
                ...
                if (strcasecmp(ext, "css") == 0) {
                    ...
                    type = malloc(strlen("image/x-icon0"));
                    ...
                    type = malloc(strlen("image/jpeg0"));
                    ...
                    type = malloc(strlen("text/x-php0"));
...
{
    char method[5], request_target[3 * LimitRequestLine], httpv[9];
    int i = 0, j = 0, abs_path_size = 0, query_size = 0;
    
    // get the method from 'line'
    for (i = 0; line[i] != ' '; ++i) {
        method[i] = line[i];
    }
    method[i] = '\0';
    ...
    if (strcasecmp(method, "GET\0") != 0) {
        error(405);
        return false;
    }
    
    // get the request-target from 'line'
    for (j = ++i; line[i] != ' '; ++i) {
        request_target[i - j] = line[i];
        
        // check for "
        if (request_target[i - j] == '\"') {
            error(400);
            return false;
        }
    }
    
    // append '\0' to request_target
    request_target[i - j] = '\0';
    
    // check first character of 'request_target'
    if (request_target[0] != '/') {
        error(501);
        return false;
    }
    
    // get the HTTP version from 'line'
    for (j = ++i; (line[i] != '\r') && (line[i + 1] != '\n'); ++i) {
        httpv[i - j] = line[i];
    }
    ...
    // check if 'httpv' is "HTTP/1.1"
    if (strcmp(httpv, "HTTP/1.1\0") != 0) {
        error(505);
        return false;
    }
    
    // get the size of absolute path
    for (i = 0; (request_target[i] != '?') && (request_target[i] != '\0'); ++i);
    abs_path_size = i;
    
    //store absolute path in abs_path
    strncpy(abs_path, request_target, abs_path_size);
    
    // append '\0' to abs_path
    abs_path[abs_path_size] = '\0';
    
    // get the size of query, if exists
    if (request_target[abs_path_size] == '?') {
        for (i = ++abs_path_size; request_target[i - 1] != '\0'; ++i);
        query_size = i - abs_path_size;
        
        // store query into 'query'
        for (i = abs_path_size; request_target[i - 1] != '\0'; ++i) {
            query[i - abs_path_size] = request_target[i];
        }
    }
