...
// copy 512 bytes from ROM (disk) into RAM (buff)
  uint8_t *rom = (uint8_t *)(EDISK_ADDR_MIN + (512 * sector));
...
// you can use Flash_FastWrite or Flash_WriteArray
  ...
  // perform fast write, if possible
  /*
  if ((addr % 128) == 0) {
    for (int i = 0; i < 4; ++i) {
      Flash_FastWrite((uint32_t *)(buff + (i * 128)), addr + (i * 128), 32);
    }
  }
  
  // else, simply write it as an array
  else {
    Flash_WriteArray((uint32_t *)buff, addr, 128);
  }*/
  Flash_WriteArray((uint32_t *)buff, addr, 128);
